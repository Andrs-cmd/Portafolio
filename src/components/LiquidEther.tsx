...
const defaultColors = ['#f4f1f1', '#747272', '#000000'];
...
`;
  ...
  const forceMultiplier = props.mouse_force || 0;

  // 🔥 fuerza continua (no se corta)
  const forceX = Mouse.diff.x * forceMultiplier;
  const forceY = Mouse.diff.y * forceMultiplier;
  ...
  const centerX = Math.min(
    Math.max(Mouse.coords.x, -1 + cursorSizeX),
    1 - cursorSizeX
  );

  const centerY = Math.min(
    Math.max(Mouse.coords.y, -1 + cursorSizeY),
    1 - cursorSizeY
  );
